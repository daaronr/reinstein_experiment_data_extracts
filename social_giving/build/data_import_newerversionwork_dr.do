**Todo: need to sort out partner's contribution
*-- which variable refers to what, in which session?
*what is the variable "ContributionPartnerR "?  it takes a -1 value sometimes

*Not sure this inputs the "old" data correctly!  
*For example, variables such as "geschlecht" (gender) are missing
...
**NOTE-- Todo: need to change this to first input old files (in a separate folder) and then new ones, and then merge these -- the commands below lead to variables being dropped

*global DRdropbox "C:\Documents and Settings\david reinstein\My Documents\My Dropbox"

**First input "old" files ** 
cd "../raw_data"
...
*Q: is this correct? Why did the variable change in the middle of 2009 runs?
*Todo: this should be checked by comparing this variable to the contribution of the reporting partner (PartnerReport)

*TODO: perhaps the best thing to do would be to manually merge things to get reported partner's donation and MaG partner's donation -- but can we have confidence in these identity variables?

rename MaGpartner MaGPartner0
gen Dem_Age=.
gen Dem_Semester=.
*These variables are truly missing for older data

order session Subject Treatment re_task treat_let report before identity Contribution Belief_average_contr ContributionPartnerR0 Belief_Partner MaGPartner0 PartnerReport  female Dem_Age Dem_Semester 

drop Time*  

drop if Treatment==0 
*Just a programming thing
*just needed this to run
...
save DataJenafirstwave, replace

*___________________________________

**Import newer files***
cd "${dropbox}\Givingexperiments\social_influence_experiment\Data\Jena_new\Data"
clear
...
cd "${dropbox}\Givingexperiments\social_influence_experiment\Data\AllJena"
save DataJenasecondwave, replace

append using DataJenafirstwave
drop if Treatment==10

replace  MaGPartner0=Partner if MaGPartner0==.
tab MaGPartner0
*Q: why is this unevenly distributed?

rename ContributionPartner ContributionPartner2009
**Q: what is this variable "ContributionPartner" for 2009 data? 


drop if Contribution<0
save Jenabothwaves, replace 
tab Contribution
*Q: why is there a value of -1 (subject 1 in session 100616_1235) 
**NEED TO DROP THIS SUBJECT!!
...
drop PrevDonation
*??what is this -- this is the same as ContributionPartnerR0 for earlier runs
*rename PartnerDonation PartDon09

*egen Reprec = rsum(ContributionPartnerR PartDon09)
...
save Jena_merged2010.dta, replace
...
save Jena_all_2010.dta, replace
...
save JenaMG_all_2010.dta, replace	

use Jena_all_2010, replace
order session Subject
	
capture drop _merge
merge session Subject using JenaMG_all_2010, unique sort keep(femalePartner ContrMaGPartner)
keep if _merge==3
save Jena_all_2010_wMaG, replace 

*testing -- this works
tab ContrMaGPartner ContributionPartnerR0 if  Treatment==23
...
save JenaPR_2010, replace 

use Jena_all_2010_wMaG, replace 
gen Subject_PR=PartnerReport
recode Subject_PR 0=.
order session Subject_PR
capture drop _merge
merge  m:1 session Subject_PR using JenaPR_2010
keep if _merge~=2
...
*At Essex we gave a list of name of persons within the lab, while at Jena we conducted a meet and greet (M&G) stage
...
gen lab=0
cd "${dropbox}\Givingexperiments\social_influence_experiment\Data\AllJena"
save berkeleysocialdata2010work, replace 
